...
class AES():
    ...
    def __init__ ( self , keyfile :str ) -> None :
        ...
        self.words = self.gen_key_schedule_256(read_key)                        #Generate key schedule from the keys

    # encrypt - method performs AES encryption on the plaintext 
    # and writes the ciphertext to disk for HW 5

    # Inputs : bv (BitVector) - file containing plaintext
    # Return : encrypted_bv (BitVector) - file contianing encrypted bv
    def encrypt_image ( self , bv ) -> None :                                   
        if(bv.size != 128):                                                 #If the block size is not 128, pad it so the size is 128 bits
        ...
        for i in range(4):                                                  #Slicing block to fill elements of statearray
            for j in range(4):
                statearray[j][i] = bv[32*i + 8*j:32*i + 8*j+8]
        statearray = self.addroundkey(statearray, self.words, 0)            #Initial XOR with the first four words
        for round_num in range (1, 15):                                     #For rounds 1 - 13, input the state array through subbytes, 
            ...
            statearray = self.addroundkey(statearray, self.words, round_num)
    ...
    def ctr_aes_image ( self , iv , image_file , enc_image ):
        ...
        * Method returns void
        ...
        line_num = 0
        while(line_num < 3):                                                    #Bypass the 3 header lines by reading new line characters
            read = bv.read_bits_from_file(8)
            read.write_to_file(output)
            nl_character = read.get_bitvector_in_ascii()
            if(nl_character == '\n'):
                line_num += 1
        while(bv.more_to_read):                                                 #Until the entire image is read
            block = bv.read_bits_from_file(128)                                 #Read in block sizes of 128
            block_encrypt = self.encrypt_image(iv)                              #Encrypt iv
            ctb = block ^ block_encrypt                                         #XOR computation
            ctb.write_to_file(output)                                           #Write to output after each block is encrypted
            num = int(iv.int_val() + 1) % (2**128)
            iv = BitVector(intVal = num, size = 128)
    ...
    def x931 ( self , v0 , dt , totalNum , outfile ):
        ...
        * Method returns void
        ...
        Vj = v0                                                                     #Set variable equal to v0
        for num in range(totalNum):
            encrypt_dt = self.encrypt_image(dt)                                     #Encryption of date and time
            rand_num = self.encrypt_image((encrypt_dt ^ Vj))                        #Encryption of date and time XORd with vj
            Vj = self.encrypt_image((rand_num ^ encrypt_dt))                        #Encryption of random number and date and time
            output.write(str(int(rand_num.get_hex_string_from_bitvector(),16)))     #Write to file
            output.write('\n')       
